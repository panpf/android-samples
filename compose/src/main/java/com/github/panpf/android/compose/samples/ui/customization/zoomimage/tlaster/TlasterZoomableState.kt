package com.github.panpf.android.compose.samples.ui.customization.zoomimage.tlaster
...
/**
 * Create a [TlasterZoomableState] that is remembered across compositions.
 *
 ...
 *
 * @param minScale the minimum scale value for [TlasterZoomableState.minScale]
 * @param maxScale the maximum scale value for [TlasterZoomableState.maxScale]
 */
@Composable
fun rememberTlasterZoomableState(
    @FloatRange(from = 0.0) minScale: Float = 1f,
    @FloatRange(from = 0.0) maxScale: Float = Float.MAX_VALUE,
): TlasterZoomableState = rememberSaveable(
    saver = TlasterZoomableState.Saver
) {
    TlasterZoomableState(
        minScale = minScale,
        maxScale = maxScale,
    )
}
...
/**
 * A state object that can be hoisted to observe scale and translate for [TlasterZoomable].
 *
 * In most cases, this will be created via [rememberTlasterZoomableState].
 *
 * @param minScale the minimum scale value for [TlasterZoomableState.minScale]
 * @param maxScale the maximum scale value for [TlasterZoomableState.maxScale]
 * @param initialTranslateX the initial translateX value for [TlasterZoomableState.translateX]
 * @param initialTranslateY the initial translateY value for [TlasterZoomableState.translateY]
 * @param initialScale the initial scale value for [TlasterZoomableState.scale]
 */
@Stable
class TlasterZoomableState(
    ...
    /**
     * The current scale value for [TlasterZoomable]
     */
    ...
    /**
     * The current translateY value for [TlasterZoomable]
     */
    @get:FloatRange(from = 0.0)
    ...
    /**
     * The current translateX value for [TlasterZoomable]
     */
    @get:FloatRange(from = 0.0)
    ...
    /**
     * Instantly sets scale of [TlasterZoomable] to given [scale]
     */
    ...
    /**
     * Animates scale of [TlasterZoomable] to given [scale]
     */
    ...
    companion object {
        /**
         * The default [Saver] implementation for [TlasterZoomableState].
         */
        val Saver: Saver<TlasterZoomableState, *> = listSaver(
            save = {
                listOf(
                    it.translateX,
                    it.translateY,
